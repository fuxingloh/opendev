...
/**
 * Bun plugin that replaces `@openxyz/runtime/workspace` with a generated
 * module. The generated variant wires an `InMemoryFs` pre-populated with the
 * template's file contents, so the deployed function serves `/workspace`
 * straight from the bundle — no disk reads, no `node_modules` shipped.
 *
 * Works by resolving the runtime's `workspace` file path from the build cwd
 ...
 */
export function inMemoryWorkspacePlugin(cwd: string, vfs: string[]): BunPlugin {
  return {
    name: "openxyz-workspace-intercept",
    setup(build) {
      // Matches the runtime's workspace source on disk — workspaces resolve
      // to real paths (`packages/openxyz-runtime/workspace.ts`), symlinked
      // installs resolve similarly.
      const filter = /openxyz-runtime\/workspace\.(ts|js)$/;
      ...
      build.onLoad({ filter }, async () => {
        ...
        // mounted at `/workspace` must store `/AGENTS.md` — storing it as
        // `/workspace/AGENTS.md` makes the agent see the file at
        // `/workspace/workspace/AGENTS.md` (the prefix doubles).
        ...
        const contents = [
          `// generated by openxyz build — intercepted workspace`,
          ...
          `};`,
          ``,
          // Match the real workspace.ts shape — constructor(cwd, permission),
