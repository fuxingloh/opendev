...
import { parseAgent, type AgentDef } from "@openxyz/runtime/agents/parser";
import { parseSkill, type SkillDef } from "@openxyz/runtime/tools/skill-parser";
...
async function loadRuntime(scan: OpenXyzFiles): Promise<OpenXyzRuntime> {
  ...
  const agentsMd = t["AGENTS.md"] ? await Bun.file(abs(t["AGENTS.md"])).text() : undefined;
