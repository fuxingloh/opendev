...
/**
 * Build the chat-sdk state adapter backed by PGlite on disk.
 *
 * PGlite is Postgres compiled to WASM — no external server, single embedded
 * file store. `@chat-adapter/state-pg` targets `pg.Pool`, so we duck-type
 * a Pool around PGlite (it only calls `.query(text, params)` and `.end()`;
